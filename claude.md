...
- **Auth libraries:** no Passport, no `class-validator`. Magic-link, password (argon2id), GitHub and Google OAuth are implemented directly in `AuthService` (OAuth = two `fetch` calls per provider). Magic-link tokens live in Redis with a 15-minute TTL, single use.
